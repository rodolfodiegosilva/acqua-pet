...
import { AdminPetDetailView } from '../components/AdminPetDetailView';
...
export const AdminPetsTab: React.FC<AdminPetsTabProps> = ({
  pets,
  clients,
  selectedPetId,
  onSelectPet,
  ...
  const paginatedPets = sortedPets.slice((page - 1) * pageSize, page * pageSize);
  const selectedPet = selectedPetId ? pets.find((pet) => pet.id === selectedPetId) ?? null : null;
  const selectedTutor = selectedPet ? clients.find((client) => client.id === selectedPet.clientId) ?? null : null;
    ...
    return (
      <>
        <AdminPetDetailView
          ...
          tutor={selectedTutor}
          onBack={onBackToList}
          onEdit={() => startEditingPet(selectedPet)}
        />
        {editingPet && (
          <div
